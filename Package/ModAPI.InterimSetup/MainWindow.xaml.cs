...
namespace ModAPI.InterimSetup
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void FindExistingLauncherKit()
        {
            try
            {
                string launcherKitPath = null;

                // Check for support.info (generated by LK 1.5.11 and newer)
                string supportInfo = Environment.ExpandEnvironmentVariables(@"%appdata%\Spore ModAPI Launcher\support.info");
                if (File.Exists(supportInfo))
                {
                    foreach (var line in File.ReadLines(supportInfo))
                    {
                        const string prefix = "Launcher Kit version: ";
                        if (line.StartsWith(prefix))
                        {
                            launcherKitPath = line.Substring(prefix.Length).Trim();
                        }
                    }
                }

                // Check for path.info (generated by LK 1.5.10 and older)
                ...
                if (launcherKitPath != null && VerifyPath(launcherKitPath, true))
                {
